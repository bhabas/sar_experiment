...
#define CANNOT_REMOVE_COLUMN "Cannot remove matrix column . The value should be less than .\n" 

#define CANNOT_REMOVE_ROW "Cannot remove matrix row . The value should be less than .\n" 
...
#define CANNOT_SWAP_ROWS \
     "Cannot swap rows (, ) because the matrix number of rows is .\n" \

#define CANNOT_SWAP_COLUMNS \
      "Cannot swap columns (, ) because the matrix number or columns is .\n" \

#define CANNOT_ROW_MULTIPLY \
      "Cannot multiply row (), maximum number of rows is .\n" \

#define CANNOT_COL_MULTIPLY "Cannot multiply col (), maximum number of columns is .\n" 
  
#define CANNOT_ADD_TO_ROW \
      "Cannot add x (row=) to row=. Total number of rows is: .\n" \

#define CANNOT_LU_MATRIX_SQUARE \
      "Canot LU. Matrix (, ) needs to be square.\n" \
...
#define CANNOT_SOLVE_LIN_SYS_INVALID_B \
      "Cannot solve system. b[][] should have size b[][].\n" \

#define CANNOT_SET_DIAG \
      "Cannot set diag with value(=). Matrix is not square.\n" \

#define CANNOT_CONCATENATE_H \
      "Cannot concatenate. Matrices have a different number of rows. Expected , found: .\n" \

#define CANNOT_CONCATENATE_V \
      "Cannot concatenate. Matrices have a different number of cols. Expected , found: .\n" \

#define CANNOT_GET_COLUMN \
      "Cannot get column (). The matrix has  number of columns.\n" \

#define CANNOT_GET_ROW \
      "Cannot get row (). The matrix has  number of rows.\n" \

#define INCONSITENT_ARRAY \
      "Cannot found element  in the array (NULL). Expected a total of :  elements.\n"  \

#define INCONSITENT_VARGS \
      "Cannot find element  in the varargs. Expecteda total of :  varargs.\n" \

#define CANNOT_REF_MATRIX_DEGENERATE \
      "Cannot compute REF. Matrix is degenerate or near degenerate.\n" \

#define CANNOT_OPEN_FILE "Cannot open file ''. Please check the path is correct and you have reading rights.\n"

#define INVALID_MATRIX_FILE \
      "Invalid matrix file: . Cannot read data.\n" \

#define VECTOR_J_DEGENERATE \
      "Vector on colum  is generate or near degenerate. Cannot proceed further.\n" \

#define CANNOT_QR_NON_SQUARE \
      "We cannot QA non-square matrix[, ].\n" \

#define CANNOT_COLUMN_L2NORM \
      "Cannot get column (). The matrix has  numbers of columns.\n" \

#define CANNOT_VECT_DOT_DIMENSIONS \
      "The two vectors have different dimensions:  and .\n" \
...
int nml_mat_row_mult_r(nml_mat *m, int row, double num) {
  if (row>= m->num_rows) {
    NML_FERROR(CANNOT_ROW_MULTIPLY);
    return 0;
  }
  int i;
...
int nml_mat_col_mult_r(nml_mat *m, int col, double num) {
  if (col>=m->num_cols) {
    NML_FERROR(CANNOT_COL_MULTIPLY);
    return 0;
  }
  int i;
  for(i = 0; i < m->num_rows; i++) {
...
nml_mat *nml_mat_col_rem(nml_mat *m, int column) {
  if(column >= m->num_cols) {
    NML_FERROR(CANNOT_REMOVE_COLUMN);
    return NULL;
  }
...
nml_mat *nml_mat_cath(int mnum, nml_mat **marr) {
  if (0==mnum) {
    return NULL;
  }
  if (1==mnum) {
  ...
  for(k = 1; k < mnum; k++) {
    if (NULL == marr[k]) {
      NML_FERROR(INCONSITENT_ARRAY);
      return NULL;
    }
    if (lrow != marr[k]->num_rows) {
      NML_FERROR(CANNOT_CONCATENATE_H);
      return NULL;
    }
...
// be variable
nml_mat *nml_mat_catv(int mnum, nml_mat **marr) {
  if (0 == mnum) {
    return NULL;
  }
  if (1 == mnum) {
  ...
  for(i = 0; i < mnum; i++) {
    ...
    if (lcol != marr[i]->num_cols) {
      NML_FERROR(CANNOT_CONCATENATE_V);
      return NULL;
    }
...
int _nml_mat_pivotidx(nml_mat *m, int col, int row) {
...
int _nml_mat_pivotmaxidx(nml_mat *m, int col, int row) {
  int i, maxi;
  double micol;
...
nml_mat_lup *nml_mat_lup_solve(nml_mat *m) {
  if (!m->is_square) {
    NML_FERROR(CANNOT_LU_MATRIX_SQUARE);
    return NULL;
  }
...
double nml_vect_dot(nml_mat *m1, int m1col, nml_mat *m2, int m2col) {
  if (m1->num_rows!=m2->num_rows) {
    NML_FERROR(CANNOT_VECT_DOT_DIMENSIONS);
  }
...
double nml_mat_col_l2norm(nml_mat *m, int col) {
  if (col >= m->num_cols) {
    NML_FERROR(CANNOT_COLUMN_L2NORM);
  }
...
int nml_mat_normalize_r(nml_mat *m) {
  ...
  for(j = 0; j < m->num_cols; j++) {
    if (l2norms->data[0][j] < NML_MIN_COEF) {
      NML_FERROR(VECTOR_J_DEGENERATE);
